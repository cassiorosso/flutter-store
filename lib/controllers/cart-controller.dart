...
abstract class _CartControllerBase with Store {
  ...
  @action
  void addCart(ProductModel product) { //Add a new product if it's not been added to the cart
    ...
    if (_sameProduct == false) {
      CartModel newProduct =
          CartModel(product: product, quantity: 1, price: product.price);
      cart.add(newProduct);
    }
  }
  ...
  Future<String> finishOrder(String paymentMethod) async {
    String customerToken = _user.user.token;
    OrderModel order =
        OrderModel(items: cart, shipPrice: shipPrice, totalPrice: totalPrice, paymentMethod: paymentMethod);
    return await _repository.makeOrder(order, customerToken).then((value) {
      if (value != null)
        return value;
      else
        return null;
